...
mod tests {
    ...
    fn test_ext_gcd() {
        ...
        for (a, b, expected_d) in test_cases {
            let (d, x, y) = ext_gcd(a, b);
            assert_eq!(d, expected_d);
            assert_eq!((a as i64) * x + (b as i64) * y, d as i64);
        }
    }
    ...
    fn test_modinv() {
        ...
        let test_cases = vec![
            (3, 11, 4),   // 3^(-1) ≡ 4 (mod 11)
            (5, 17, 7),   // 5^(-1) ≡ 7 (mod 17)
            (7, 13, 2),   // 7^(-1) ≡ 11 (mod 13)
            (10, 17, 12), // 10^(-1) ≡ 12 (mod 17)
            (15, 26, 7),  // 15^(-1) ≡ 7 (mod 26)
        ];

        for (a, m, expected) in test_cases {
            let result = modinv(a, m);
            assert_eq!(result, expected);
            assert_eq!((a * result) % m, 1);
        }
    }
}
